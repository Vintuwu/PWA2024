// Este archivo sirve para dado un json que contenga los imdbId de peliculas/series, los busca usando una API key gratuita de hasta 1000 request diarias, y los guarda en un archivo nuevo asi no hay que estar constantemente haciendo request a la API
...
async function fetchMovieData(movie) {
  return new Promise((resolve, reject) => {
    https.get(`https://www.omdbapi.com/?i=${movie.imdbId}&plot=full&apikey=38635810`, (res) => {
      let data = '';
...
(async () => {
  try {
    const allMoviesData = await fetchAllMoviesData(movies);
    fs.writeFileSync('movie_dataAnimation.json', JSON.stringify(allMoviesData, null, 2));
    console.log('Movie data saved to movie_data.json');
  } catch (error) {
